...
const createMessage = async (req, res) => {
    try{
        const {recipient,type,messageData} = req.body;
        let msgArray = [];
        
        if(type === 1){
            msgArray = recipient.split(',');
        }else if(type === 2){
        ...
        for (let valofmsgarray of msgArray){
            // console.log(valofmsgarray);
            // console.log(type);
            if(type === 1){
                const {message, referenceNumber} = req.body
                const recipientTwo = valofmsgarray;
            }else if(type === 2){
                const {recipientTwo, message, referenceNumber} = valofmsgarray;
            }
            console.log(recipientTwo);
            console.log(message);
...
//                     } else{
//                         numberError.push(valofrecipientarr); 
...
//             return res.status(400).json({status:400,error : 'These number is not registered', numberError})
//         }else{
