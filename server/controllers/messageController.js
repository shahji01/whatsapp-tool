const { phoneNumberFormatter,compareDate } = require('../helpers/formatter')
const messageModel = require('../models/messageModel')
const {fsReadAndWrite,sendReq} = require('../helpers/commonFunction.js')
const { log } = require('console');
const companyModel = require('../models/companyModel');
const path = './jsons/dbMessage.json';
const url = require('url');
const { MessageMedia } = require('whatsapp-web.js');

const sendImage = async (req, res) => {
    return console.log(req.instance);
    const client = await global.session.find(sess => sess.id == req.instance._id)?.client
    return console.log(client);
    const base64 = "";
    const image = await new MessageMedia("image/jpeg", base64, "image.jpg");
    client.sendMessage("923145017055@c.us", image, {caption: "My image JPG!"});
}

const createMessage = async (req, res) => {
    
    try{
        const {recipientTwo,type,messageData} = req.body;
        let msgArray = [];
        let msgResult = [];
        let sendResponse = '';
        
        if(type === 1){
            msgArray = recipientTwo.split(',');
        }else if(type === 2){
            
            if (messageData.length == undefined) {
                msgArray.push(messageData);
            } else {
                msgArray = messageData;
            }
        }
        for (let valofmsgarray of msgArray){
            if(type === 1){
                const {message, referenceNumber} = req.body
                const recipient = valofmsgarray;
                let emptyFields = []

                if (!recipient) {
                    emptyFields.push('recipient')
                }
                
                if (!message) {
                    emptyFields.push('messages')
                }
                if (!referenceNumber) {
                    emptyFields.push('referenceNumber')
                }

                if (emptyFields.length > 0) {
                    return res.status(400).json({status:400,error : 'Please fill in all the fields', emptyFields})
                }
                sendResponse = await insertMessageDB(recipient,message,referenceNumber,req.instance._id,req.instance.phone,req.instance.companyId);
            }else if(type === 2){
                const {recipient, message, referenceNumber} = valofmsgarray;
                let emptyFields = []

                if (!recipient) {
                    emptyFields.push('recipient')
                }
                
                if (!message) {
                    emptyFields.push('messages')
                }
                if (!referenceNumber) {
                    emptyFields.push('referenceNumber')
                }

                if (emptyFields.length > 0) {
                    return res.status(400).json({status:400,error : 'Please fill in all the fields', emptyFields})
                }
                sendResponse = await insertMessageDB(recipient,message,referenceNumber,req.instance._id,req.instance.phone,req.instance.companyId);
            }
            msgResult.push(sendResponse.data);
        }
        return res.status(200).json({status:200,data:msgResult});
    }catch (err) {
        return res.status(400).json({status:400,error:err});
    }
}
async function insertMessageDB (recipient, message, referenceNumber,instanceId,instancePhone,instanceCompanyId){
    const client = global.session.find(sess => sess.id == instanceId)?.client
    const number = phoneNumberFormatter(recipient)
    try {
        const isRegisteredNumber = await client.isRegisteredUser(number)
        if(isRegisteredNumber){
            const response = await messageModel.create({companyId: instanceCompanyId, instanceId: instanceId, sender: instancePhone, recipient, message , userRefId: referenceNumber,whatsappRefId: '' , status: 'Pending'})
            return {status:200,data: response};
        } else{ 
            return {status:400,data:{companyId:instanceCompanyId,instanceId:instanceId,sender:instancePhone,recipient:recipient,message:message,userRefId:referenceNumber,status:'False',msg:'This number is not registered'}};
        }  
    } catch (err) {
        return {status:400,error:'Please try again once the whatsapp status is ready'};
        //return res.status(400).json({status:400,error : 'Please try again once the whatsapp status is ready'})
    }
}


async function instancePendingMsg (columnName,type){
    const getInstancePendingMsg = await messageModel.aggregate([
        {
            $match: {[columnName]: type}
        },
        {
          $group: {
            _id: "$instanceId",
            doc: {$first: "$$ROOT"},
          }
        },
        {
          $replaceRoot: {newRoot: "$doc"}
        }
      ]);
      return getInstancePendingMsg;
}

async function insertMessage(response,msgId) {            
    const whatsappId = response.id.id
    const WhatsappStatus = waStatus(response.ack)
    try{
        await messageModel.findByIdAndUpdate(msgId, { status: WhatsappStatus,whatsappRefId:whatsappId,webhookStatus:'false' });    
        console.log('Success');
    } catch (err) {
        console.log(err);
    }
}

const sendInstanceMessage = async () => {
    try{
        const getInstancePendingMsg = await instancePendingMsg('status','Pending');
        if(getInstancePendingMsg.length !== 0){
                getInstancePendingMsg.forEach(msgDetail => {
                    const client = global.session.find(sess => sess.id == msgDetail.instanceId)?.client
                    const number = phoneNumberFormatter(msgDetail.recipient)
                    client.sendMessage(number, msgDetail.message).then(response => {                    
                        insertMessage(response,msgDetail._id);
                    }).catch(err => {   
                        console.log(err);
                    })
                });
                console.log('Run Send Instance Msg');
        }
        
    } catch (err){
        console.log({error : 'Something went Wrong! Please try again'});
    }
}

const sendWebhookMessage = async () => {
    let updateWebHookRes = []
    let sendResponst = [];
    try{
        //const pendingWebHookMsg = await instancePendingMsg('webhookStatus','false')
        const companyIds = await messageModel.find({'webhookStatus' : false}).distinct('companyId');
        if(companyIds.length !== 0){
            for (let valofCompanyId of companyIds) {
                const companyMsgs = await messageModel.find({'companyId':valofCompanyId,'webhookStatus' : false});
                for(let valofCompanyMsg of companyMsgs){
                    const updateMsgStatus = await messageModel.findOneAndUpdate({whatsappRefId: valofCompanyMsg.whatsappRefId},{webhookStatus:'true'})
                    updateWebHookRes.push(updateMsgStatus);
                    sendResponst.push({
                        whatsappRefId: updateMsgStatus.whatsappRefId,
                        userRefId: updateMsgStatus.userRefId,
                        status: updateMsgStatus.status
                    });
                    fsReadAndWrite(path, updateMsgStatus);
                }

                const getCompanyDetail = await companyModel.findById(valofCompanyId);
                const parseURL = url.parse(getCompanyDetail.webhook, true) //we will fetch this from db
                const data = sendResponst;
                const options = {
                    host:parseURL.hostname,
                    port:parseURL.port,
                    path:parseURL.path,
                    method : 'POST'
                }
                sendReq(options, data)
                
            }
            console.log('Run WebHook')
        }
    } catch (err){
        console.log(err)
    }
}

const updateMsgStatus = async (req, res) => {
    const {id, referenceNumber, status} = req.body;
    const WhatsappStatus = waStatus(status);
    const updateMsgStatus = await messageModel.findOneAndUpdate({whatsappRefId: id},{status: WhatsappStatus,webhookStatus:'false'})
    return res.status(200).json(updateMsgStatus)
}

const instanceWiseMsgList = async (req, res) => {
    try{

        const {authorization} = req.headers;
        if (!authorization) {
            return res.status(401).json({status:401,error:'Authorization token required...'})
        }
        const token = authorization.split(' ')[1]
        const getInstanceDetail = await companyModel.find({'instances.token' : token});
        var result = await getInstanceDetail[0].instances?.find(item => item.token === token);
        const instanceId = result._id;
        const data = await messageModel.find({'instanceId':instanceId})
        return res.status(200).json({status:200,data:data});
    } catch (err){
        return res.status(400).json({status:400, error:err.message});
    }
}

const waStatus = (code) => {
    switch (code) {
        case -1:
            return 'Error'
        case 0:
            return 'Sent'
        case 1:
            return 'Sent'
        case 2:
            return 'Delivered'
        case 3:
            return 'Read'
        default:
            return 'Invalid'
    }
}

module.exports = { createMessage,updateMsgStatus,sendInstanceMessage,sendWebhookMessage,instanceWiseMsgList,sendImage } 